...
def save_outputs(sampler, out_prefix, indep, denoised_xyz_stack, px0_xyz_stack, seq_stack, xyz_particle, seq_particle, Lasu):
    ...
    # Determine default resis to use if we are overwriting seqs generated by diffusion
    if (sampler.inf_conf.set_diffused_prot_to is not None) and (sampler.inf_conf.set_diffused_prot_to in rf2aa.chemical.num2aa):
        default_prot_res_num = rf2aa.chemical.aa2num[sampler.inf_conf.set_diffused_prot_to]
    else:
        default_prot_res_num = 0

    if (sampler.inf_conf.set_diffused_dna_to is not None) and (sampler.inf_conf.set_diffused_dna_to in rf2aa.chemical.num2aa):
        default_dna_res_num = rf2aa.chemical.aa2num[sampler.inf_conf.set_diffused_dna_to]
    else:
        default_dna_res_num = 22

    if (sampler.inf_conf.set_diffused_rna_to is not None) and (sampler.inf_conf.set_diffused_rna_to in rf2aa.chemical.num2aa):
        default_rna_res_num = rf2aa.chemical.aa2num[sampler.inf_conf.set_diffused_rna_to]
    else:
        default_rna_res_num = 27

    # replace mask and unknown tokens in the final seq: 
    # for protein chains, replace with alanine
    pro_condition_01 = ((final_seq == 20) | (final_seq==21)) & indep.is_protein
    final_seq = torch.where(pro_condition_01, default_prot_res_num, final_seq)
    # for DNA chains, replace with deoxyribo-adenine
    dna_condition_01 = ((final_seq == 26) | (final_seq==21)) & indep.is_dna
    final_seq = torch.where(dna_condition_01, default_dna_res_num, final_seq)
    # for RNA chains, replace with ribo-adenine
    rna_condition_01 = ((final_seq == 31) | (final_seq==21)) & indep.is_rna
    ...
    # for protein chains, replace diffused with alanine
    if (sampler.inf_conf.set_diffused_prot_to is not None):
        print('SETTING PROT SEQ TO ALANINE!!!')
    ...
    # for DNA chains, replace diffused with deoxyribo-adenine
    if (sampler.inf_conf.set_diffused_dna_to is not None):
        print('SETTING DNA  SEQ TO ADENINE!!!')
    ...
    # for RNA chains, replace diffused with ribo-adenine
    if (sampler.inf_conf.set_diffused_rna_to is not None):
        print('SETTING RNA  SEQ TO ADENINE!!!')
