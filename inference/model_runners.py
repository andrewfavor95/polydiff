...
class Sampler:
    ...
    def sample_init(self, return_forward_trajectory=False):
        ...
        if self.diffuser_conf.partial_T:
            ...
            assert indep.xyz.shape[0] ==  L, f"there must be a coordinate in the input PDB for each residue implied by the contig string for partial diffusion.  length of input PDB != length of contig string: {indep.xyz.shape[0]} != {L}"
