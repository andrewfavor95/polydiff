...
class Trainer():
    ...
    def calc_loss(self, logit_s, label_s,
                  ...
                  pred_in, pred_tors, pred_allatom, true, mask_crds, mask_BB, mask_2d, same_chain,
                  pred_lddt, idx, bond_feats, dataset, chosen_task, t, xyz_in, diffusion_mask,
        ...
        loss_weights = {
            ...
            'inter_fape'                : w_inter_fape*disp_tscale,
            'bond_loss'                 : w_bond*disp_tscale,
            'atom_bond_loss'            : w_atom_bond*disp_tscale,
            'skip_bond_loss'            : w_skip_bond*disp_tscale,
            'rigid_loss'                : w_rigid*disp_tscale,
            'clash_loss'                : w_clash*disp_tscale,
        }
        ...
        # FAPE on NA secondary structure residue-pairs
        # if ss_mask_2d.sum() > 0:
        #     had_ss_shown = True
        if had_ss_shown:
        ...
        # if (pred_xyz_fa.shape[0] > 1):
            
        #     # bond_loss = rf2aa.loss.calc_cart_bonded(seq, pred_allatom[1:], idx, self.cb_len, self.cb_ang, self.cb_tor)
        #     bond_loss = rf2aa.loss.calc_cart_bonded(seq, pred_xyz_fa[1:], idx, self.cb_len, self.cb_ang, self.cb_tor).mean()
        #     # if w_bond > 0.0:
        #     #     tot_loss += w_bond*bond_loss.mean()
        #     # loss_dict['clash_loss'] = ( bond_loss.detach() )
        # else:
        #     bond_loss = torch.tensor(0).to(device=pred.device)
