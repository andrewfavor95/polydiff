...
def save_outputs(sampler, out_prefix, indep, denoised_xyz_stack, px0_xyz_stack, seq_stack, xyz_particle, seq_particle, Lasu):
    ...
    first_seq = torch.argmax(seq_stack[0],dim=-1)
    bfacts = torch.ones_like(final_seq.squeeze())


    if (sampler.inf_conf.set_diffused_prot_to is not None) :
        if len(sampler.inf_conf.set_diffused_prot_to)==2:
            sampler.inf_conf.set_diffused_prot_to = f' {sampler.inf_conf.set_diffused_prot_to}'

        if (sampler.inf_conf.set_diffused_prot_to in rf2aa.chemical.num2aa):
    ...
    if (sampler.inf_conf.set_diffused_dna_to is not None) :
        if len(sampler.inf_conf.set_diffused_dna_to)==2:
            sampler.inf_conf.set_diffused_dna_to = f' {sampler.inf_conf.set_diffused_dna_to}'

        if (sampler.inf_conf.set_diffused_dna_to in rf2aa.chemical.num2aa):
    ...
    if (sampler.inf_conf.set_diffused_rna_to is not None) :
        if len(sampler.inf_conf.set_diffused_rna_to)==2:
            sampler.inf_conf.set_diffused_rna_to = f' {sampler.inf_conf.set_diffused_rna_to}'

        if  (sampler.inf_conf.set_diffused_rna_to in rf2aa.chemical.num2aa):
